...
; (function (window, document, $) {
  ...
  $(window).on("load", function () {
    ...
    $(".btnretona").on("click", function () {

      var page = $(this).val();
      var ajaxurl = '/app/login/wizard_return';
      var data = {page:page};
      $.post(ajaxurl, data, function(response) {
        ...
        $("#page" + response.page).removeClass('hidden');
    })
    ...
    //Custom File Input
    $("#enviar-logo input").change(function (e) {
      ...
      formData.append("fornecedor", fornecedor);
       ...
       console.log(formData);
         $.ajax({
             url: '/app/painel/enviarlogofornecedor',
             type: 'POST',
             data: formData,
             beforeSend: function () {
               $("#status2").html(' - ENVIANDO!!');
             },
  ...
  $(document).ready(function () {
    ...
    $("#page" + $('#pageid').val()).removeClass('hidden');
    ...
    var frmsend1 = $(".formsend");
    frmsend1.submit(function (e) {
      e.preventDefault();
      enviarFomulario(frmsend1);

    });

    var frmsend2 = $(".formsend2");
    frmsend2.submit(function (e) {
      e.preventDefault();
      enviarFomulario(frmsend2);

    });



    function enviarFomulario (frmsend) {
      ...
      var data = '';
        ...
        $.ajax({
            type: frmsend.attr('method'),
            url: frmsend.attr('action'),
            data: frmsend.serialize(),
            beforeSend: function () {
              $('.beforeload').html("<span class='spinner-border spinner-border-sm' role='status' aria-hidden='true'></span>");
            },
            success: function (data) {
              ...
              console.log(data);
                ...
                if (data.codigo == '1') {
                  ...
                  if (data.btn != undefined && data.btn != '') {
                    Swal.fire(
                      {	text: data.msg,
                        title: data.title,
                  ...
                  for(var i=0; i<len; i++){
                    ...
                    if (data.lista[i].valida === 'S'){ 
                      item_valida = '<span class="badge badge-success badge-pill badge-round ml-1"><i class="bx bx-check"></i></span>';
                    } else {
                      item_valida = '<span class="badge badge-warning badge-pill badge-round ml-1"><i class="bx bx-x"></i></span>';
                    }
                      item_lista = '<li class="list-group-item d-flex justify-content-between align-items-center">' +
                                      '<span> ' + data.lista[i].descricao + '</span>' + item_valida + 
                                   '</li>';
  
                    $("#ListaModal").append(item_lista);
    ...
    var frmexcluir = $(".formexcluir");
    frmexcluir.submit(function (e) {

      var data = '';

        e.preventDefault();
        $.ajax({
            type: frmexcluir.attr('method'),
            url: frmexcluir.attr('action'),
            data: frmexcluir.serialize(),
            beforeSend: function () {
              $('.beforeload').html("<span class='spinner-border spinner-border-sm' role='status' aria-hidden='true'></span>");
            },
            success: function (data) {
    ...
    */
    ...
    $("#up_vid").on('change',function() {
     var formData = new FormData();
     var iddoc = $("#iddoc").val();
     var file = $("#up_vid")[0].files[0];
     ...
     formData.append("iddoc", iddoc);
      ...
      console.log(formData);
        $.ajax({
            url: '/app/painel/enviardocumentoup',
            type: 'POST',
            data: formData,
            beforeSend: function () {
  ...
  $("#cpfcnpj_checkbox").click(function() {
    if($('#cpfcnpj_checkbox').is(':checked')) { 
        $('.cnpj').show();
        $('.cpf').hide();
        $('.cpf').html('');
    } else {
...
$("#ValidaCpf").on("keyup", function (e) {
  ...
  if (cpfval.length >= 14) {
    ...
    if (cpf(cpfval) === true) {
      ...
      //document.getElementById("CpfValidado").disabled = false;
    } else {
      $(this).removeClass("is-valid");
      $(this).addClass("is-invalid");
      //document.getElementById("CpfValidado").disabled = true;
    }
...
function SomarLinhas(item) {
  var quant = $('#quantitem' + item).html();
  quant = quant.replace(/,/g, '.');
  var valor = $('#valor' + item).val();
  valor = valor.replace(/,/g, '.');
  /* var valor_edit = $('#valor' + item).val();
  valor_edit = valor_edit.replace(/([0-9]),([0-9]{2}$)/g, "$1,$2");
  $('#valor' + item).attr( 'value', valor_edit); */

  //$('#totalitem' + item).html(formatter.format(quant * valor));
  $('#totalitem' + item).val(formatter.format(quant * valor));
}

function tougleDiv(div) {
...
function SalvarLinha (item, ncoletacrip, chave) {

  var valor_item = $('#valor' + item).val();
  var marca_item = $('#marca' + item).val();

  if (valor_item == ''){
    document.getElementById('valor'+item).focus();
  }

  var linha = document.getElementById('valor' + item);

  var id_item = linha.dataset.item
  var id_item_pro = linha.dataset.itemPro; 
  var id_item_obriga = linha.dataset.itemObriga; 

  if ((id_item_obriga == 'S') && (marca_item == '')) {
    document.getElementById('marca'+item).focus();
    return false;
  }

  $.ajax({
    type: 'POST',
    url: '/app/login/salvaritem',
    dataType: 'json',
    data: { 
            item:id_item, 
            ncoletacrip:ncoletacrip, 
            chave:chave, 
            valor_item:valor_item, 
            marca_item:marca_item,
            id_item_pro:id_item_pro,
            id_item_obriga:id_item_obriga
          },
          beforeSend: function(){
            $('#result' + item).html('<i class="bx bx-loader bx-spin bx-md success"></i>');
         },
    success: function (data) {
      console.log(data);

      if (data.codigo == '1' ) {
        $('#result' + item).html('<i class="bx bxs-like bx-tada-hover bx-md success"></i>');
        if (data.retorno != '' ) {
          $('#valor' + item).attr('data-item', data.retorno);
        }
      } else {
        $('#result' + item).html('<i class="bx bxs-error bx-flashing bx-md danger"></i>');
      }
